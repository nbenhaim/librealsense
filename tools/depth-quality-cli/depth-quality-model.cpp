...
namespace rs2
{
    namespace depth_quality
    {
        ...
        void tool_model::render()
        {
            ...
            try
            {
                int plane_not_found_counter = 0;
                //_metrics_model.process_frames(_pipe);
                while(true)
                {
                    _viewer_model.roi_rect = _metrics_model.get_plane();
                    ...
                    _metrics_model.process_frame(f, plane_not_found_counter);
        ...
        void tool_model::update_configuration()
        {
            ...
            //****THIS LINE IS FOR LATEST VERSION OF LIBREALSENSE 
        ...
        {
            // printf("begin metrics model loop\n");
            // while (_active)
            // {
        ...
        void metrics_model::process_frames(pipeline p)
        {
        ...
        void metrics_model::process_frame(rs2::depth_frame &f, int & plane_not_found_counter)
        {
            ...
            if ((RS2_STREAM_DEPTH == stream_type) && (RS2_FORMAT_Z16 == stream_format))
            {
                ...
                if( plane_fit_set)
                {
                    plane_not_found_counter = 0;
                   
                }
                else
                {
                    plane_not_found_counter++;
                }
                
                
                if(plane_not_found_counter == 20)
                {
                    printf("Point the realsense at a wall about 2 meters away and hold it still & perpendicular\n");
                }
